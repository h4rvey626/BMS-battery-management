...
#define MCU_WAKE_BQ_ONOFF(x)      HAL_GPIO_WritePin(GPIOA, MCU_WAKE_BQ, (x) ? GPIO_PIN_SET : GPIO_PIN_RESET)
...
/* 软件保护阈值 (区别于硬件寄存器阈值) */
#define OV_SOFT_THRESHOLD     4200  // 软件过压触发 (mV)
#define OV_SOFT_RECOVERY      4100  // 软件过压恢复 (mV)
#define UV_SOFT_THRESHOLD     2800  // 软件欠压触发 (mV)
#define UV_SOFT_RECOVERY      2900  // 软件欠压恢复 (mV), 需高于触发值形成回差
#define OC_THRESHOLD_MA       2000  // 过流阈值 (mA)
#define BALANCE_THRESHOLD_MV  30    // 平衡启动压差 (mV)

/* 外部变量声明 */
extern int Batteryval[50];
extern unsigned char DSG_STA;
extern unsigned char CHG_STA;

/* 保护相关外部变量 (volatile: 多任务读写) */
extern volatile unsigned char OV_FLAG;
extern volatile unsigned char UV_FLAG;
extern volatile unsigned char OC_FLAG;
extern volatile unsigned char Temp_up_flag;
extern float Tempval_2;

/* 保护函数声明 */
int BMS_Protection_Control(int *cell_voltages, int current, float temperature);

/* 报警标志外部变量声明 */
extern int UV_Alarm_flag;
extern int OV_Alarm_flag;
extern int SCD_Alarm_flag;
extern int OCD_Alarm_flag;
extern volatile int OT_Alarm_flag;
extern volatile int UT_Alarm_flag;
